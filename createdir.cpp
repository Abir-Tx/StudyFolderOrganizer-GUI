...
void Createdir::rootDirCreator(QString &driveLetter, QString &year,
  ...
  rootDir = driveLetter + "University Study/Study Materials/" + year +
            " Year/" + semester + " Semester/" + labOrTheory + "/";
  dir.setPath(rootDir);
  dir.mkpath(rootDir);
  if (dir.exists()) {
...
QString Createdir::labOrtheory(bool isLabSelected, bool isTheorySelected) {
  if (isLabSelected)
    return "Lab";
  else if (isTheorySelected)
    return "Theory";
  else
    return 0;
}

// Creates Subject folders based on user's given names & adds mid-term &
// final-term folders inside each sub folder
